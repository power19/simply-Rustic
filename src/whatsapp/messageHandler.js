...
function welcomeText() {
  return WELCOME.replace('{business}', process.env.BUSINESS_NAME || 'Simply Rustic Catering');
}
...
async function formatItemList(categoryId) {
  ...
  const lines = items.map((item, i) => `${i + 1}) ${item.name} - ${money(item.price)}${item.description ? `\n   _${item.description}_` : ''}`);
  const text = [
...
async function formatServiceList() {
  ...
  const lines = services.map(
    (s, i) => `${i + 1}) *${s.name}*${s.price ? ` - from ${money(s.price)}` : ''}${s.description ? `\n   _${s.description}_` : ''}`
  );
...
async function handleMessage(client, message) {
  ...
  if (lower === 'cart') {
    return message.reply(formatCart(session.cart));
  }
  if (lower === 'checkout' && session.step !== 'CHECKOUT_NOTE') {
    if (!session.cart.length) {
      return message.reply('Your cart is empty. Type *menu* to browse our menu first.');
    }
    await saveSession(phone, { step: 'CHECKOUT_NOTE', categoryId: null, cart: session.cart });
  ...
  switch (session.step) {
    case 'MAIN': {
      if (lower === '1') {
        await saveSession(phone, { step: 'BROWSING_CATEGORIES', cart: session.cart });
        return message.reply(await formatCategoryList());
      }
      if (lower === '2') {
        await saveSession(phone, { step: 'BROWSING_SERVICES', cart: session.cart });
    ...
    case 'BROWSING_CATEGORIES': {
      ...
      await saveSession(phone, { step: 'BROWSING_ITEMS', categoryId: category.id, cart: session.cart });
    ...
    case 'BROWSING_ITEMS': {
      ...
      await saveSession(phone, { step: 'BROWSING_ITEMS', categoryId: session.categoryId, cart });
    ...
    case 'BROWSING_SERVICES': {
      ...
      await saveSession(phone, { step: 'SERVICE_NOTE', categoryId: service.id, cart: session.cart });
      return message.reply(
    ...
    case 'CHECKOUT_NOTE': {
      ...
      return message.reply(
        `Thank you! Your order #${order.id} for ${money(total)} has been received. We'll confirm it with you shortly.\n\nType *menu* to place another order.`
      );
    }
